...
agent none{
   stage('git checkout'){
      try {
      git credentialsId: 'git', url: 'https://github.com/Npraveenreddy/image.git'
      } catch(err) {
   ...
   stage('artifacts to s3') {
      ...
      withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AKIATIB5K2VXUEWQ7HOU', credentialsId: 'aws', secretKeyVariable: '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']]) {
         sh "aws s3 ls"
